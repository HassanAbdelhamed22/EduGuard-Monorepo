...
import { getStudntQuiz, viewRegisteredCourses } from '../../services/studentService';
...
const Dashboard = () => {
  ...
  const fetchData = async () => {
    setIsLoading(true);
    try {
      const courseReg = await viewRegisteredCourses();
      setCourses(Array.isArray(courseReg?.data) ? courseReg.data : []);

      const quizReg = await getStudntQuiz();
    ...
    <div className='p-2 md:p-6 min-h-screen grid grid-cols-1 lg:grid-cols-6 gap-4'>
            ...
            <div className="grid grid-cols-1 gap-4">
              {courses.map((course, index) => (
                <div key={course.CourseID}
                  className={`p-6 rounded-lg shadow-md hover:shadow-lg transition-shadow duration-200 ${colors[index % colors.length]}`}
                >
                  <div className="flex items-center gap-4">
                    <div className="p-3 bg-blue-50 rounded-full">
                      <Book className={`w-6 h-6 ${iconColors[index % iconColors.length]}`} />
                    </div>
                    <div>
                      <h4 className="text-lg font-semibold text-white">{course.CourseName}</h4>
                      <p className="text-sm text-white/85">Code: {course.CourseCode}</p>
      ...
      <div className='md:col-span-2'>
        <div className='bg-white p-2 rounded-lg shadow-md grid grid-cols-1 gap-4'>
          ...
          <div className='mt-8'>
            <h3 className='text-xl font-semibold text-gray-800 mb-4'>Upcoming Quizzes</h3>
            {nearestQuizzes.length === 0 ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-1 gap-4"> 
                {nearestQuizzes.map((quiz) => (
                  <div key={quiz.QuizID} className="p-6 rounded-lg border border-gray-200 hover:shadow-lg transition-shadow duration-200 bg-gray-50">
                    <div className="flex items-center gap-4"> 
                      <div className='p-3 bg-blue-50 rounded-full'>
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-700">Course: {quiz.CourseName}</p>
                        <p className="text-sm text-gray-600">Date: {quiz.QuizDate}</p>
                        <div className="flex gap-4 text-[13px]">
                          <p className='text-gray-500'>From: {new Date(quiz.StartTime).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</p>
                          <p className="text-gray-500">To: {new Date(quiz.EndTime).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</p>
                        </div>
                      </div>
                    </div>
                  </div>
